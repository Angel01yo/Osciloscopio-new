...
// Version 2.3 - Corregido definitivo
// ============================================================================
...
static void usb_send_waveform(float *samples, int count) {
    int len = 0;
    len += snprintf(usb_tx_buffer + len, USB_BUFFER_SIZE - len, "{\"t\":\"wf\",\"s\":[");
    ...
    len += snprintf(usb_tx_buffer + len, USB_BUFFER_SIZE - len, "]}");
...
static void usb_send_fft_compact(void) {
    int len = 0;
    len += snprintf(usb_tx_buffer + len, USB_BUFFER_SIZE - len, "{\"t\":\"ft\",\"f\":[");
    ...
    len += snprintf(usb_tx_buffer + len, USB_BUFFER_SIZE - len, "],\"m\":[");
    for (int i = 0; i < FFT_HALF_SIZE && i < 32; i++) {
...
int main(void) {
    ...
    oled_text(15, 32, "PICO v2.3", true);
    ...
    printf("\n");
    printf("========================================\n");
    printf("  OSCILOSCOPIO PICO v2.3 - USB OTG\n");
    printf("========================================\n");
    printf("Conecta GPIO 22 -> GPIO 26 para prueba\n");
